...
public class SwipeActionAdapter extends DecoratorAdapter implements
	...
	protected SparseArray<OnGetViewListener> mOnGetViewListeners = new SparseArray<OnGetViewListener>();
	...
	public View getView(final int position, final View convertView,
		...
		if (output == null) {
			...
			for (int i = 0; i < mBackgroundResIds.size(); i++) {
				output.addBackground(
						View.inflate(parent.getContext(),
								mBackgroundResIds.valueAt(i), null),
						mBackgroundResIds.keyAt(i));
				OnGetViewListener listener = mOnGetViewListeners.get(
						mBackgroundResIds.keyAt(i), null);
				if (listener != null) {
					output.registerOnGetView(listener,
							mBackgroundResIds.keyAt(i));
				}
			}
		...
		output.setContentView(super.getView(position, output.getContentView(),
				output));
		if (hasActions(position)) {
			output.onGetView(position, convertView, parent, this);
		}
	...
	/**
	 * SwipeActionTouchListener.ActionCallbacks callback We just link it through
	 * to our own interface
	 *
	 * @param position
	 *            the position of the item that was swiped
	 * @return boolean indicating whether the item has actions
	 */
	@Override
	public boolean hasActions(int position) {
		return mSwipeActionListener != null
				&& mSwipeActionListener.hasActions(position);
	}
	 ...
	 * @param position
	 *            The position to perform the action on, sorted in descending
	 *            order for convenience.
	 * @param direction
	 *            The type of swipe that triggered the action
	 * @return boolean that indicates whether the list item should be dismissed
	 *         or shown again.
	 */
	@Override
	public boolean onPreAction(ListView listView, int position, int direction) {
		return mSwipeActionListener != null
				&& mSwipeActionListener.shouldDismiss(position, direction);
	}
	 ...
	 *            The originating {@link ListView}.
	 * @param position
	 *            The positions to perform the action on, sorted in descending
	 *            order for convenience.
	 * @param direction
	 *            The type of swipe that triggered the action.
	 */
	...
	public void onAction(ListView listView, int[] position, int[] direction) {
		if (mSwipeActionListener != null) {
			mSwipeActionListener.onSwipe(position, direction);
		}
	}
	 ...
	 *
	 * @param mFadeOut
	 *            true makes items fade out with a swipe (opacity -> 0)
	 * @return A reference to the current instance so that commands can be
	 *         chained
	 */
	...
	/**
	 * Set whether the backgrounds should be fixed or swipe in from the side The
	 * default value for this property is false: backgrounds will swipe in
	 *
	 * @param fixedBackgrounds
	 *            true for fixed backgrounds, false for swipe in
	 */
	public SwipeActionAdapter setFixedBackgrounds(boolean fixedBackgrounds) {
		this.mFixedBackgrounds = fixedBackgrounds;
		if (mListView != null)
			mTouchListener.setFixedBackgrounds(fixedBackgrounds);
		return this;
	}

	/**
	 * Set whether the backgrounds should be dimmed when in no-trigger zone The
	 * default value for this property is false: backgrounds will not dim
	 *
	 * @param dimBackgrounds
	 *            true for dimmed backgrounds, false for no opacity change
	 */
	public SwipeActionAdapter setDimBackgrounds(boolean dimBackgrounds) {
		this.mDimBackgrounds = dimBackgrounds;
		if (mListView != null)
			mTouchListener.setDimBackgrounds(dimBackgrounds);
		return this;
	}

	/**
	 * Set the fraction of the View Width that needs to be swiped before it is
	 * counted as a far swipe
	 *
	 * @param farSwipeFraction
	...
	/**
	 * Set the fraction of the View Width that needs to be swiped before it is
	 * counted as a normal swipe
	 *
	 * @param normalSwipeFraction
	 *            float between 0 and 1
	 */
	 ...
	 *
	 * @param listView
	 *            the ListView to which the adapter will be attached
	 * @return A reference to the current instance so that commands can be
	 *         chained
	 */
	...
	/**
	 * Add a background image for a certain callback. The key for the background
	 * must be one of the directions from the SwipeDirections class.
	 *
	 * @param key
	 *            the identifier of the callback for which this resource should
	 *            be shown
	 * @param resId
	 *            the resource Id of the background to add
	 * @return A reference to the current instance so that commands can be
	 *         chained
	 */
	...
	public SwipeActionAdapter addBackground(int key, int resId,
		...
		if (SwipeDirections.getAllDirections().contains(key)) {
			mBackgroundResIds.put(key, resId);
			mOnGetViewListeners.put(key, getViewListener);
		}
		return this;
	}

	public SwipeActionAdapter addOnGetViewListener(int direction,
			OnGetViewListener getViewListener) {
		if (SwipeDirections.getAllDirections().contains(direction)) {
			mOnGetViewListeners.put(direction, getViewListener);
		}
		return this;
	}
	 ...
	 *
	 * @param mSwipeActionListener
	 *            class listening to swipe events
	 * @return A reference to the current instance so that commands can be
	 *         chained
	 */
	public SwipeActionAdapter setSwipeActionListener(
			SwipeActionListener mSwipeActionListener) {
	...
	/**
	 * 
	 * Interface for getView callback.
	 * Don't forget to cache View.findViewById in tag and/or viewholders
	 * @author Damien Degois
	 *
	 */
	public static interface OnGetViewListener {
		public void onGetView(int position, int direction, View innerView,
				SwipeViewGroup swipeViewGroup, BaseAdapter adapter);
	}
