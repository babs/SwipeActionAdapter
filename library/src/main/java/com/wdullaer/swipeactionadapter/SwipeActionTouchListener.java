 ...
 * {@link SwipeActionTouchListener} is paused during list view scrolling.
 * </p>
 *
 * <p>
 * Example usage:
 ...
 *
 * <pre>
 * SwipeActionTouchListener touchListener = new SwipeActionTouchListener(listView,
...
public class SwipeActionTouchListener implements View.OnTouchListener {
	...
	/**
	 * The callback interface used by {@link SwipeActionTouchListener} to inform
	...
	public interface ActionCallbacks {
		...
		/**
		 * Called when the user has swiped a list item position. The listener
		 * will wait for this method to return before starting the dismiss
		 * animation or the reappear animation. Please perform any heavy
		 * computations in an ASyncTask to avoid blocking the interface.
		 *
		 * @param listView
		 *            The originating {@link ListView}.
		 * @param position
		 *            The position to perform the action on, sorted in
		 *            descending order for convenience.
		 * @param direction
		 *            The type of swipe that triggered the action
		 * @return boolean that indicates whether the list item should be
		 *         dismissed or shown again.
		 */
		boolean onPreAction(ListView listView, int position, int direction);

		/**
		 * Called after the dismiss or reappear animation of a swiped item has
		 * finished.
		 *
		 * @param listView
		 *            The originating {@link ListView}.
	 ...
	 * {@link SwipeActionTouchListener} is paused during list view
	 * scrolling.</p>
	 *
	...
	/**
	 * Set whether the list item should fade out when swiping or not. The
	 * default value for this property is false
	 *
	 * @param fadeOut
	 *            true for a fade out, false for no fade out.
	 */
	...
	/**
	 * Set whether the backgrounds should be dimmed while in no-trigger zone The
	 ...
	 *            float between 0 and 1, should be equal to or greater than
	 *            normalSwipeFraction
	 */
	 ...
	 *            float between 0 and 1, should be equal to or less than
	 *            farSwipeFraction
	 */
		...
		switch (MotionEventCompat.getActionMasked(motionEvent)) {
				...
				for (int i = 0; i < childCount; i++) {
					...
					if (rect.contains(x, y)) {
						try {
							...
							mDownView = mFixedBackgrounds ? mDownViewGroup
									.getContentView() : child;
							if (!mFixedBackgrounds)
								mDownViewGroup.translateBackgrounds();
						} catch (Exception e) {
							mDownView = child;
						}
						break;
			...
			case MotionEvent.ACTION_CANCEL: {
				...
				if (mDownView != null && mSwiping) {
					...
					ViewPropertyAnimator.animate(mDownView).translationX(0)
							.alpha(1).setDuration(mAnimationTime)
							.setListener(null);
				}
			...
			case MotionEvent.ACTION_UP: {
				...
				if (Math.abs(deltaX) > (mViewWidth * mNormalSwipeFraction)
						&& mSwiping) {
					dismiss = true;
				...
				if (dismiss && mDownPosition != ListView.INVALID_POSITION) {
					...
					ViewPropertyAnimator
							.animate(mDownView)
							.translationX(
									-ViewHelper.getTranslationX(mDownView))
							.alpha(mFadeOut ? 0 : 1)
							.setDuration(mAnimationTime)
							.setListener(new AnimatorListener() {
								@Override
								public void onAnimationEnd(Animator animation) {
									boolean performDismiss = mCallbacks
											.onPreAction(mListView,
													downPosition, direction);
									if (performDismiss) {
										performDismiss(downView, downPosition,
												direction);
									} else {
										slideBack(downView, downPosition,
												direction);
									}
								}
			...
			case MotionEvent.ACTION_MOVE: {
				...
				if (mSwiping) {
					...
					mDownViewGroup.showBackground(mDirection, mDimBackgrounds
							&& (Math.abs(deltaX) < mViewWidth
	...
	private void slideBack(final View slideInView, final int downPosition,
			final int direction) {
		mPendingDismisses.add(new PendingDismissData(downPosition, direction,
				slideInView));

		ViewHelper.setTranslationX(slideInView,
				ViewHelper.getTranslationX(slideInView));
		ViewPropertyAnimator.animate(slideInView).translationX(0).alpha(1)
				.setDuration(mAnimationTime)
				.setListener(createAnimatorListener(slideInView.getHeight()));
	}

	private void performDismiss(final View dismissView,
			final int dismissPosition, final int direction) {
		...
		mPendingDismisses.add(new PendingDismissData(dismissPosition,
				direction, dismissView));
		animator.start();
	}

	private AnimatorListener createAnimatorListener(final int originalHeight) {
		return new AnimatorListener() {
			...
			public void onAnimationEnd(Animator animation) {
				...
				if (mDismissAnimationRefCount == 0) {
					...
					mCallbacks.onAction(mListView, dismissPositions,
							dismissDirections);
